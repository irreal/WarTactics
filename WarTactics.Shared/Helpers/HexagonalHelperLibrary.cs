// Here be dragons! Be warned! Try not to touch any of this, even though it looks like it really needs touching.
namespace WarTactics.Shared.Helpers
{
    ...
    public struct PointD
    {
        ...
        public static implicit operator Vector2(PointD p)
        {
            return new Vector2((float)p.x, (float)p.y);
        }

        public static implicit operator PointD(Vector2 v)
        {
    ...
    public struct Hex
    {
        ...
        public static List<Hex> Range(Hex source, int range)
        {
            var hexesInRange = new List<Hex>();

            for (int i = -range; i <= range; i++)
            {
                for (int u = Math.Max(-range, -i - range); u <= Math.Min(range, -i + range); u++)
                {
                    var z = -i - u;
                    hexesInRange.Add(Add(source, new Hex(i, u, z)));
                }
            }
    ...
    public struct Layout
    {
        ...
        static public Orientation PointDy = new Orientation(Math.Sqrt(3.0), Math.Sqrt(3.0) / 2.0, 0.0, 3.0 / 2.0, Math.Sqrt(3.0) / 3.0, -1.0 / 3.0, 0.0, 2.0 / 3.0, 0.5);
